#!chezscheme
;;; pregexp.sls -- Dorai Sitaram's pregexp, ported to Chez Scheme R6RS library
;;; Based on the version included in Gerbil Scheme (with vyzo's modifications)
;;; Definition order: leaf helpers first, then mutually-recursive parser core
...
(library (compat pregexp)
  ...
  (define *pregexp-return-char*
    (integer->char (+ 13 *pregexp-nul-char-int*)))
  ...
  (define-syntax pregexp-error
    (syntax-rules ()
      ((_ where irritants ...)
       (error where "pregexp internal error" irritants ...))))

  ;; --- Leaf helpers (no forward references) ---
  ...
  ;; --- Mutually recursive parser core ---
  ;; These 4 functions form a cycle: read-pattern → read-branch → read-piece
  ;; → read-subpattern → read-pattern. We use letrec to handle the mutual
  ;; recursion cleanly, avoiding forward references in the library body.
  ;; Only pregexp-read-pattern is needed outside this group.
  ...
  (define pregexp-match-positions
    (case-lambda
      ((pat str)
       (pregexp-match-positions pat str #f #f))
      ((pat str start)
       (pregexp-match-positions pat str start #f))
      ((pat str start end)
        ...
        (let loop ((i 0) (r ""))
          (if (>= i n) r
              (let ((pp (pregexp-match-positions pat str i n)))
                (if (not pp)
                    (if (= i 0) str
                        (string-append r (substring str i n)))
                    (loop (cdar pp)
                          (string-append
                           r
